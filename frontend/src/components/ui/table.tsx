...
export interface TableProps extends React.HTMLAttributes<HTMLTableElement> {
  /** Classes for the scroll wrapper rather than the table element itself. */
  wrapperClassName?: string;
}

const Table = React.forwardRef<HTMLTableElement, TableProps>(
  ({ className, wrapperClassName, ...props }, ref) => (
    // A wide table scrolls inside itself so it can never widen the page.
...
>(({ className, ...props }, ref) => (
  <tr
    ref={ref}
    className={cn(
      "border-b border-border transition-colors last:border-0 hover:bg-muted/50 data-[state=selected]:bg-accent",
      className,
    )}
    {...props}
  />
