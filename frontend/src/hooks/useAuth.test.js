...
function AuthProbe() {
    ...
    return (_jsxs("div", { children: [_jsx("p", { "data-testid": "status", children: isLoading ? "загрузка" : user ? `вход выполнен: ${user.name}` : "вход не выполнен" }), _jsx("button", { type: "button", onClick: () => {
                    void login({ email: OLYA.email, password: "Demo1234!" });
                }, children: "\u0412\u043E\u0439\u0442\u0438" }), _jsx("button", { type: "button", onClick: () => {
...
describe("AuthProvider", () => {
    it("сообщает, что вход не выполнен, когда /auth/me отвечает 401", async () => {
        ...
        renderWithProviders(_jsx(AuthProbe, {}));
    ...
    it("выполняет вход через login() без повторного запроса", async () => {
        ...
        stubFetch((url) => {
            if (url === "/api/auth/me")
                return errorResponse(401, "Требуется вход");
            if (url === "/api/auth/login")
                return jsonResponse(OLYA);
    ...
    it("сбрасывает весь кеш при выходе, чтобы данные не попали в следующую сессию", async () => {
        ...
        const { queryClient } = renderWithProviders(_jsx(AuthProbe, {}));
        await waitFor(() => expect(status()).toBe("вход выполнен: Оля"));
    ...
    it("очищает клиент, даже если запрос выхода завершился ошибкой", async () => {
        const user = userEvent.setup();
        stubFetch((url) => url === "/api/auth/logout"
